package pl.karoldronia.artefacts.config;
...
import pl.karoldronia.artefacts.artefact.item.ArtefactItemSerializer;
    ...
    public <T extends OkaeriConfig> T load(Class<T> configClass, File path, String fileName) {
        return ConfigManager.create(configClass, (config) -> {
            ...
            config.withSerdesPack(serders -> {
                serders.register(new ArtefactItemSerializer());
            });
